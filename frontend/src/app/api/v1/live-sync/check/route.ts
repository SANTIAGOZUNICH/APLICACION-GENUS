 ...
 * Si cambió: parse SEMANAS + proyección en la misma request (autoritativo).
 * No requiere GET /work-items posterior.
 */
export async function GET(request: NextRequest) {
  ...
  const sector = (searchParams.get("sector") as SectorId | null) ?? "ELABORACION";
  const ownerPerson = searchParams.get("ownerPerson");
  ...
  try {
    ...
    return NextResponse.json({
      ...
      sector,
      ownerPerson: ownerPerson ?? undefined,
      date: date ?? undefined,
